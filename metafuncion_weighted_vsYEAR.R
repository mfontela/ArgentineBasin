metafuncion_weighted_vsYEAR<-function(table,tracer, weight){
...
model<-(table%>% #Fusilo summary
          MASS::rlm(tracer[layer==unique(layer)[i]] ~ G2year[layer==unique(layer)[i]],., weights=(1/weight[layer==unique(layer)[i]]))%>%
          summary())
